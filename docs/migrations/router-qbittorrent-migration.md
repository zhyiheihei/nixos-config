...
> 2026-08-12 规范化：复用公共 `qbittorrent.nix` / `qbittorrent-pt.nix` /
> `qbittorrent-seedbox.nix` 模块，router 的 `hosts/router/qbittorrent.nix` 只做目录
> 映射、激活门闩、主机级覆盖（官方客户端、固定端口、单接口绑定、WebUI 白名单）
> 与 vhost 别名，不再新增公共模块。
