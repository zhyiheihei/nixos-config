...
- `sgvm` 接管逻辑主机名 `greencloud`，保留新加坡节点的硬件、SSH host key、
  ZeroTier 客户端身份和公网地址。
- 保留作者 `greencloud` 的服务角色，并合并原 `sgvm` 的 AI 与监控服务。
...
- 作者原 greencloud 的 Gitea、Matrix、Maddy、Miniflux、NetBox、
...
1. 构建 `greencloud` 和 `ml-home-vm`，验证 SOPS 解密。
2. 不停机预同步应用数据和数据库快照。
3. 停止旧机应用服务、数据库、WireGuard 和 ZeroTier controller。
4. 做最后一次增量同步和数据库导出。
5. 部署新 `greencloud`，初始化数据库用户和空数据库。
6. 导入数据库，启动并逐项验证服务。
...
- 迁移前 `greencloud` 与 `sgvm` 的 NixOS 服务模块并集已全部由新
  `greencloud` 导入。
