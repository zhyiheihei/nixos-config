...
| `api`、`cal`、`element`、`git`、`id`、`login`、`n8n`、`posts`、`rss`、`stats`、`tools`、`whois` | CNAME 到 `jp.zhyi.cc` | JP VPS TLS SNI 入口；`n8n` 最终转发至 `ml-home-vm` |
...
上述公开入口统一静态指向 `jpvm`，不配置自动故障转移。`twvm` 保留其独立服务
与手动 VLESS 备用订阅，但不承接正常的公开服务流量。
