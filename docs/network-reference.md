...
| `*.ml-home-vm.zhyi.cc` | CNAME 到 `home-ddns.zhyi.cc` | 未单独列出的 `ml-home-vm` 私有 HTTPS，标准端口 `8443` |
| `homepage.ml-home-vm.zhyi.cc`、`archivebox.ml-home-vm.zhyi.cc`、`syncthing.ml-home-vm.zhyi.cc`、`halo.ml-home-vm.zhyi.cc`、`linkwarden.ml-home-vm.zhyi.cc`、`excalidraw.ml-home-vm.zhyi.cc`、`freshrss.ml-home-vm.zhyi.cc`、`memos.ml-home-vm.zhyi.cc`、`vertex.ml-home-vm.zhyi.cc` | CNAME 到 `cnvm.zhyi.cc` | `cnvm:443` 的 TLS SNI 转发至 colocrossing LTNET 入口，再转发到 `ml-home-vm:8443`；正式入口受 OAuth 保护 |
| `ha.zhyi.cc` | CNAME 到 `cnvm.zhyi.cc` | CNVM 的 TLS SNI 转发至家庭服务 |
| `hydra.zhyi.cc` | CNAME 到 `cnvm.zhyi.cc` | Hydra vhost 反代到 `pve-5700u` 的 Hydra 端口 |
| `sub.zhyi.cc` | CNAME 到 `cnvm.zhyi.cc` | 标准 `443` 经 CNVM 与 colocrossing 转发至 `ml-home-vm:8443`，订阅地址由本机服务生成并受 OAuth 保护 |
...
| `*.zhyi.xin` | A `101.96.199.157` | CNVM 公网入口的兜底记录；下列 CNAME 例外优先 |
| `ai`、`attic`、`gemini`、`lemmy`、`mail`、`matrix`、`pb`、`rsshub` 等 `zhyi.xin` 服务 | CNAME 到 `home-ddns.zhyi.cc` | 家庭公网入口 |
| `api`、`cal`、`element`、`git`、`id`、`login`、`n8n`、`posts`、`rss`、`stats`、`tools`、`whois` | CNAME 到 `cnvm.zhyi.cc` | CNVM TLS SNI 入口；`n8n` 最终转发至 `ml-home-vm` |
