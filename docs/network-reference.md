...
| `homepage.ml-home-vm.zhyi.cc`、`archivebox.ml-home-vm.zhyi.cc`、`syncthing.ml-home-vm.zhyi.cc`、`halo.ml-home-vm.zhyi.cc`、`linkwarden.ml-home-vm.zhyi.cc`、`excalidraw.ml-home-vm.zhyi.cc`、`freshrss.ml-home-vm.zhyi.cc`、`memos.ml-home-vm.zhyi.cc`、`vertex.ml-home-vm.zhyi.cc` | CNAME 到 `tw.zhyi.cc` | `twvm:443` 的 TLS SNI 转发至 colocrossing LTNET 入口，再转发到 `ml-home-vm:8443`；正式入口受 OAuth 保护 |
...
| `sub.zhyi.cc` | CNAME 到 `tw.zhyi.cc` | 标准 `443` 经 VPS 与 colocrossing 转发至 `ml-home-vm:8443`，订阅地址由本机服务生成并受 OAuth 保护 |
...
| `api`、`cal`、`element`、`git`、`id`、`login`、`n8n`、`posts`、`rss`、`stats`、`tools`、`whois` | CNAME 到 `tw.zhyi.cc` | VPS TLS SNI 入口；`n8n` 最终转发至 `ml-home-vm` |
...
当前 Gcore 免费套餐不支持为同一个名称配置多条动态 GEO 记录。因此上述公开
入口目前静态指向 `twvm`，不是 `jpvm -> twvm` 自动故障转移。若需主备，应先
准备支持多记录健康路由的 DNS 服务，再修改 DNSControl；不要以删除现有记录的
方式试探套餐限制。
