...
默认二级路由一般会把 LAN 流量 NAT 到上级网段。这个配置会让二级网段客户端出公网最稳妥，所以默认建议保留：
...
示例里 `wan` zone 是 `firewall.@zone[1]`，实际操作前先确认：
...
- 二级网段访问互联网正常
- 二级网段访问一级网段正常，但一级网段看到的源地址可能是二级路由上级口 `192.168.2.20`
- 一级网段访问二级网段仍然可以通过静态路由和防火墙放行实现

只有在明确需要“无 NAT 互访”，并且确认一级路由会为 `192.168.3.0/24` 做出公网 NAT / 转发时，才考虑关闭二级 NAT。否则二级网段客户端可能无法访问互联网。
...
- 如果二级网段客户端不能上网，二级路由 `wan` zone 是否保留了 `masq='1'`
- 如果要求无 NAT，一级路由是否会为 `192.168.3.0/24` 做出公网 NAT / 转发
...
后续发现关闭二级 NAT 会导致二级网段客户端无法访问互联网，因此实际保留了：
