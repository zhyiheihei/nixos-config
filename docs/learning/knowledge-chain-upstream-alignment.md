...
| 私有同步 | Syncthing 客户端 + 服务器 | ml-2700 与 opi5p 已配对，Notes 文件夹双向 in sync |
...
| 博客评论 | Waline `comments.lantian.pub` | Waline 已在 colocrossing 运行，`comments.zhyi.xin` DNS 待发布 |
...
- Notes 已迁到 Syncthing 持久目录，`~/Documents/Notes` 通过 bindfs 暴露；
  Syncthing 与 `opi5p` 配对，双方 `rest/db/status?folder=notes` 均为 idle、
...
- 在 GitHub 创建 `zhyiheihei/blog`（当前账号 token 无效，无法用 API 代建），
  然后推送并启用 Pages。
- Waline 已部署到 colocrossing 并启动；`comments.zhyi.xin` DNS 记录待发布。
