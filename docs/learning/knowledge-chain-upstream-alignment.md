...
| 私有同步 | Syncthing 客户端 + 服务器 | 已生效：ml-2700 / opi5p / greencloud 三节点同步 |
...
| 内容索引 | pyison `posts.lantian.pub` | `posts.zhyi.xin` 已在 greencloud 部署 |
...
- ml-2700 / opi5p / greencloud 三节点的 Syncthing `notes` 文件夹已正常运行。
- greencloud 的 Waline 与 `comments.zhyi.xin` 相关改动已回滚。
