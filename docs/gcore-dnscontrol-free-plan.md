...
- `ha.zhyi.cc` 是唯一的动态主备记录：`jpvm` 权重 100，`twvm` 权重 1，TTL 为
  120 秒，健康检查频率为 300 秒。
- 其他原本经 `twvm` 发布的服务保留 `CNAME -> tw.zhyi.cc.`，以保持已经验证过的
  可用链路。
- 想让所有服务实现 `jpvm` 主、`twvm` 自动备用，先升级 Gcore 套餐或迁移到支持
  多条健康检查记录的 DNS 提供商；不要在免费套餐上重新批量改 GEO。
