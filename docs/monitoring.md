...
2026-07-21 的首次审计确认 `logvm` 上的 Prometheus、Alertmanager 与 Grafana 均为
`active`，采集器已能读取多数主机和服务指标。以下不是监控本身故障，而是应由网络
或服务任务处理的真实信号：

- `logvm` 到 `jpvm` 的 LTNet 地址 `198.18.0.117` 当前没有可用返回路径，导致
  `jpvm` 的 node、Nginx、BIRD、CoreDNS、WireGuard exporter 均为 `down`。
