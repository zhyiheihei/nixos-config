...
- 提交后 **push origin**：主控机为本机 `ml-laptop`，仓库对齐就是本机与 origin
  两方；`ml-builder` 不再维护仓库克隆（2026-09-05 定，历史三方对齐流程作废）
...
### 对齐质量标准：人视角对齐（2026-10 定，历次教训固化）

对齐的验收标准只有一个：**用 diff 工具打开，看不到差异**（除已登记的认可偏离）。
具体要求：

- **逐字复刻，禁止等价方案**：非认可偏离项必须与 exam 原版一模一样——同样的
  结构、同样的变量名、同样的折行空行。禁止自创「行为等价」的写法：换单位、
  换 helper、合并/展开表达式、调整语句顺序、换数据结构、加防御性默认值，
  全部算偏离。「我的写法更简洁/更现代」不是理由，行为等价 ≠ 对齐
- **代码里不写对齐注释、不打日志**：不在代码中留「对齐 exam 原版」「此处与上游
  一致」类注释，不加 log/trace/echo 调试语句。原因和依据写进
  `docs/agent/upstream-baseline.md` 的变更记录或对应 `docs/agent/` 文档，
  以及提交信息；代码本身保持与作者原版完全一致
- **对齐前先全量枚举差异**：逐文件 `diff` 出差异清单，逐项核销为
  「认可偏离 / 待对齐 / 待登记 allowlist」三类，不凭印象挑几处改
- **每个偏离都要有出处**：能落到既有认可偏离规则（域名/用户名/硬性值映射、
  exam-check-allowlist 条目）的偏离才允许存在，落不进去的要么照抄原版，
  要么先停下问用户
