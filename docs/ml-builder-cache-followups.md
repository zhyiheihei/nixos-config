...
- On `ml-builder-cache`, `attic.zhyi.cc` resolves back to the machine's LTNET
  address.
...
- Decide whether `attic.zhyi.cc:4000` should be served by this host, router
  forwarding, or another reverse proxy.
- If Nginx on this host serves `attic.zhyi.cc`, make sure the matching
