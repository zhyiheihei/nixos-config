...
- vendor r8125 驱动 A/B 状态：2026-08-11 17:40 eth0 NETDEV WATCHDOG 复现旧事故（见 [06](06-router-lan-drop-netdev-watchdog.md)），曾回滚 r8169；2026-08-12 为追 OpenWrt 高吞吐重新启用 RSS/多队列版并编译关闭 ASPM/EEE，待部署复测。eth0 的 `10-router-lan.link` 固定 EEPROM MAC 仍保留；2.5G hairpin 吞吐复测未做；“重启变关机”事故未定位。
