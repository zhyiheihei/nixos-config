...
`make four` 会同时构建 `ml-builder`、`ml-home-vm`、`ml-2700u` 和
`pve-2700u`，但不会连接远端切换系统。

`make current` 会直接部署并切换 `ml-builder`、`ml-home-vm` 和
...
nix run .#colmena -- eval \
  --on ml-builder,ml-home-vm,ml-2700u,pve-2700u
```
...
nix run .#colmena -- apply \
  --on ml-builder,ml-home-vm,ml-2700u
```
...
```bash
nix run .#colmena -- apply \
  --on ml-builder,ml-home-vm,pve-2700u
```
...
| `make current` | 部署 `ml-builder`、`ml-home-vm`、`ml-2700u` | 是 |
