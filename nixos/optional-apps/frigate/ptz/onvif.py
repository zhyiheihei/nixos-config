...
class OnvifController:
    ...
    @staticmethod
    def _recover_zeep_elements(value_obj, tag_suffix):
        """乐橙(Imou)摄像头固件返回的 ONVIF XML 元素顺序与 XSD 定义不符，
        zeep 严格按 sequence 解析导致部分元素被丢进 _raw_elements（解析为
        None/空）。这里从 _raw_elements 里把需要的元素恢复为 dict 列表。"""
        recovered = []
        raw = getattr(value_obj, "_raw_elements", None)
        if not raw:
            return recovered
        for el in raw:
            ...
            for ch in el:
                ...
                if n == "URI":
                    d["URI"] = "".join(ch.itertext())
                elif n in ("XRange", "YRange"):
                    rng = {}
                    for rr in ch:
                        rn = rr.tag.split("}")[-1] if "}" in rr.tag else rr.tag
                        try:
                            rng[rn] = float("".join(rr.itertext()))
                        except (ValueError, TypeError):
                            rng[rn] = "".join(rr.itertext())
                    d[n] = rng
                else:
                    d[n] = "".join(ch.itertext())
            recovered.append(d)
        return recovered

    @classmethod
    def _recover_zeep_scalar(cls, value_obj, tag_suffix):
        """从 _raw_elements 恢复单个标量属性（如 DefaultRelativePanTiltTranslationSpace）。"""
        for d in cls._recover_zeep_elements(value_obj, tag_suffix):
            return d.get("URI")
        return None
    ...
    async def _init_onvif(self, camera_name: str) -> bool:
        ...
        try:
            ...
            if configs.DefaultRelativePanTiltTranslationSpace is None:
                _drp = self._recover_zeep_scalar(
                    configs, "DefaultRelativePanTiltTranslationSpace"
                )
                if _drp:
                    configs.DefaultRelativePanTiltTranslationSpace = _drp
        except Exception as e:
            logger.error(
        ...
        if (
            ...
            if not ptz_config.Spaces.RelativePanTiltTranslationSpace:
                recovered = self._recover_zeep_elements(
                    ptz_config.Spaces, "RelativePanTiltTranslationSpace"
                )
                if recovered:
                    ptz_config.Spaces.RelativePanTiltTranslationSpace = recovered
                    logger.debug(
                        f"{camera_name}: recovered {len(recovered)} relative pan/tilt spaces"
                    )
