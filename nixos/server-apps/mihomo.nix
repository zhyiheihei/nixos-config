...
let
  cfg = config.lantian.mihomo;
in
{
  options.lantian.mihomo = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Enable the mihomo CN-unblock SOCKS proxy on cn-accel hosts.";
    };
  };

  config = lib.mkIf (LT.this.hasTag LT.tags.cn-accel && cfg.enable) {
